...
import SelfAssignmentModal from "../Components/SelfAssignmentModal";
import "animate.css";
...
const GroupChat = () => {
	...
	const loadAuthenticatedImage = useCallback(
		async (messageId, url) => {
			...
			for (const msg of rawMessages) {
				...
				const hasAttachment =
					...
					(msg.attachment.fileType === "image" ||
						...
						msg.attachment.originalName?.match(
	...
	}, [rawMessages, loadAuthenticatedImage, imageUrls]);
	...
	const handleFileUpload = async (e) => {
		...
		try {
			...
			await axios.post(`${API_URL}/${roomId}/upload`, formData, {
				headers: {
	...
	const handleDeleteMessage = async (messageId) => {
		...
		if (result.isConfirmed) {
			try {
				const token = localStorage.getItem("token");
				await axios.delete(`${API_URL}/${roomId}/message/${messageId}`, {
					headers: { Authorization: `Bearer ${token}` },
				});
		...
		try {
			const res = await axios.get(
				`https://tenor.googleapis.com/v2/search?q=${
					gifQuery || "funny"
				}&key=AIzaSyCoEto9P8kesHKJq7jpZq26eefogABlF1Q&limit=9&media_filter=mp4`
			);
	...
	return (
		...
		>
			...
			<div className='d-flex flex-grow-1'>
				...
				/>
				<main
					className={`d-flex flex-row flex-grow-1 content ${
						sidebarOpen ? "" : "shifted"
					}`}
					style={{
						paddingTop: "66px",
				...
				>
					...
					<div className='chat-room-view'>
						<div
							className='chat-container-wrapper d-flex flex-column w-100 border border-danger-subtle border-4 shadow-sm'
							style={{
								margin: "16px",
								marginLeft: 0,
								marginTop: "20px",
						...
						>
							{/* Chat Header */}
							<div
								className='chat-header p-3 bg-white border-bottom border-danger-subtle'
								style={{ flexShrink: 0 }}
							>
								...
								{!room?.isPrivate && room?.participants?.length > 0 && (
									<small className='text-muted'>
										...
										{room.participants.length} anonymous{" "}
							...
							{/* Self-Assignment Button - Show for Secret Santa rooms with self-assign strategy */}
							{isSecretSantaRoom &&
								room?.assignmentStrategy === "self-assign" &&
								roomStatus !== "drawn" && (
									<div
										style={{
											padding: "1rem",
									...
									>
										...
										<p className='text-muted small mt-2 mb-0'>
											Select who you'd like to gift to based on your draw
										</p>
									</div>
								)}
							...
							{/* Message Area */}
							<div
								className='d-flex flex-column animate__animated animate__fadeIn'
								style={{
									backgroundColor: "rgba(244, 244, 244, 0.7)",
									flex: "1 1 auto",
									minHeight: 0,
			...
			{imageViewer.show && (
				<ImageViewerModal
					...
					onClose={() =>
						setImageViewer({ show: false, url: null, fileName: null })
					}
				/>
			)}
			...
			{showSelfAssignModal && (
				<SelfAssignmentModal
					...
					roomData={room}
					onAssigned={() => {
						// Refresh room data after assignment
						fetchRoomData();
					}}
				/>
			)}
