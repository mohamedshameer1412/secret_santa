...
// @access  Private
router.post('/reveal', protect, asyncHandler(async (req, res) => {
...
router.get('/current', protect, asyncHandler(async (req, res) => {
  ...
  if (!child) {
    throw new AppError('No child found. Generate one first!', 404);
  }
...
// @access  Private
router.post('/:childId/clue', protect, asyncHandler(async (req, res) => {
  ...
  if (new Date() < child.nextClueAt) {
    ...
    throw new AppError(`Next clue unlocks in ${hoursLeft} hour(s)`, 400);
  }
...
// @access  Private
router.post('/:childId/dare', protect, asyncHandler(async (req, res) => {
  ...
  if (!child.dare || !child.dare.text) {
    throw new AppError('No dare assigned yet', 400);
  }
